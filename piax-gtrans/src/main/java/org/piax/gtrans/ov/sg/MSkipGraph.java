...
public class MSkipGraph<D extends Destination, K extends ComparableKey<?>>
        extends OverlayImpl<D, K> implements RoutingTableAccessor, SGExecQueryCallback {
    /*--- logger ---*/
    ...
    public MSkipGraph(TransportId transId, ChannelTransport<?> lowerTrans)
        ...
        ddllSG = new SkipGraph<Endpoint>(new TransportId(transId + "x"), 
    ...
    public FutureQueue<?> forwardQueryToMaxLessThan(ObjectId sender,
        ...
        NestedMessage nmsg = new NestedMessage(sender, receiver, null, peerId,
                msg);
        Collection<RemoteValue<?>> ret = ddllSG.forwardQuery(lu.isPlusDir(),
    ...
    public RemoteValue<?> sgExecQuery(Comparable<?> key, Object msg) {
        ...
        /*
         * TODO 複数の値を無理やりセットしている。この部分はいずれきれいにしないといけない
         */
        MVal mval = new MVal();
...
//       	OverlayListener<D, K> listener = getListener(nmsg.receiver);
    ...
    @Override
    public Link[] getAll() {
        return ddllSG.getNeighbors(null, false, 0);
    }

    @Override
    public Link getLocal(Comparable<?> key) {
        return ddllSG.getLocal(key);
    }
    ...
    public Link[] getRights(Comparable<?> key, int level) {
        return ddllSG.getNeighbors(key, true, level);
    }

    @Override
    public Link[] getLefts(Comparable<?> key, int level) {
        return ddllSG.getNeighbors(key, false, level);
    }
