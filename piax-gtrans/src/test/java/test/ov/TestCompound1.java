...
/**
 * LLNetとSimpleFloodingの間でCompoundOverlayのテストを行う
 * 
 * @author     Mikio Yoshida
 * @version
 */
public class TestCompound1 extends Common {
    ...
    public static void main(String[] args) throws IOException, IdConflictException {
        ...
        List<TransportIdPath> ovIdPaths = 
                Arrays.asList(new TransportIdPath("llnet"), new TransportIdPath("flood"));
        ...
        for (int i = 0; i < numPeer; i++) {
            ...
            peers[i].peer = peer;
            peers[i].bt = bt;
            CompoundOverlay<GeoRegion, Location> comp = new CompoundOverlay<GeoRegion, Location>(
                    peer, ovIdPaths);
            ...
            if (0 <= i && i <= gateway) {
                ...
                Overlay<GeoRegion, Location> llnet = new LLNet(new TransportId("llnet"), sg);
                llnet.setListener(comp.getTransportId(), comp);
                llnet.addKey(comp.getTransportId(), new Location(i/10.0, i/10.0));
                peers[i].ov1 = llnet;
            }
            if (gateway <= i && i < numPeer) {
                Overlay<GeoRegion, Location> flood = new SimpleFlooding<GeoRegion, Location>(new TransportId("flood"), bt);
                flood.setListener(comp.getTransportId(), comp);
                flood.addKey(comp.getTransportId(), new Location(i/10.0, i/20.0));
                peers[i].ov2 = flood;
            }
